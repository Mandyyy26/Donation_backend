...
router.get("/", AdminAuth, async (req, res) => {
  try {
    ...
    return res.status(200).send({ RaisedHands: raisedHandsList });
  } catch (error) {
...
router.post("/raise-hand-on-an-item", UserAuth, async (req, res) => {
  try {
    ...
    if (!product) return res.status(400).send(messages.product_id_required);

    // Raised hand user and owner should not be same
    if (req.body.user_details._id.toString() === product.posted_by.toString())
      return res.status(400).send("You cannot raise your own hand");
    ...
    if (checkRaise) return res.status(400).send(messages.already_raised_hands);
...
router.post("/accept-raised-hand", UserAuth, async (req, res) => {
  try {
    ...
    if (!raisedHand)
      return res.status(400).send(messages.raised_hand_not_found);
    ...
    return res.send({ room_id: getChatRoom.room._id });
  } catch (error) {
    return res.status(500).send(messages.serverError);
  }
});
